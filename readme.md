## 说明 ##
《Node.js 台湾社群协作电子书》写的不错，但是关于express描述中的部分内容选取的不正确，关于express的get、post、ajax三部分内容讲的其实是使用纯node代码实现的。这么好的一本教程，少了如今大行其道的express的部分内容，总感觉像是武功秘籍有残缺一样，甚是遗憾。所以，为了将教程实现的更完整，我特意花了一天的时间来完善express部分的内容。    
